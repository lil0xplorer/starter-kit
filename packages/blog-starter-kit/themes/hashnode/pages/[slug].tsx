...
const Post = ({ publication, post, morePosts }: PostProps) => {
	...
	const breadcrumbs = [
		{ name: 'Home', url: '/' },
		{ name: 'Blog', url: '/blog' },
		{ name: post.title, url: post.url, isCurrentPage: true },
	];

	// Add tag breadcrumb if post has tags
...
const Page = ({ page, publication }: PageProps) => {
	const breadcrumbs = [
		{ name: 'Home', url: '/' },
		{ name: page.title, url: `/${page.slug}`, isCurrentPage: true },
	];
	...
	return (
		<>
			<SEOHead
				...
				page={page}
				pageType="page"
				currentUrl={`${publication.url}/${page.slug}`}
				breadcrumbs={breadcrumbs}
			/>
