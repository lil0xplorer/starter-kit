...
var separator_root_1 = require("./separator-root");
var post_floating_bar_tooltip_wrapper_1 = require("./post-floating-bar-tooltip-wrapper");
var toc_sheet_1 = require("./toc-sheet");
var post_share_widget_1 = require("./post-share-widget");
function PostFloatingMenu(props) {
    var isPublicationPost = props.isPublicationPost, post = props.post, shareText = props.shareText, showPaymentModal = props.showPaymentModal, openComments = props.openComments, list = props.list;
    ...
    var commentBtnAccessibleLabel = (post === null || post === void 0 ? void 0 : post.responseCount) > 0
        ? "".concat((0, image_1.kFormatter)(post.responseCount + (post.replyCount || 0)), " comment").concat(post.responseCount === 1 ? '' : 's', ", open the comments")
        : 'Open comments';
    ...
    dangerouslySetInnerHTML={{
      ...
      <div className="post-floating-bar fixed left-0 right-0 z-50 flex h-12 w-full flex-wrap justify-center 2xl:h-14">
        <div className="relative mx-auto flex h-12 shrink flex-wrap items-center justify-center rounded-full border-1/2 border-slate-200 bg-white px-5 py-1 text-sm  text-slate-800 shadow-xl dark:border-slate-500 dark:bg-slate-700 dark:text-slate-50 2xl:h-14">
            <post_floating_bar_tooltip_wrapper_1.default label="Write a comment">
            {post && (<div>
                <button type="button" onClick={openComments} aria-label={commentBtnAccessibleLabel} className="outline-none! flex cursor-pointer items-center rounded-full hover:bg-slate-100 dark:hover:bg-slate-800">
                  <>
                    <span className="rounded-full p-2">
                      <svgs_1.CommentSVGV2 className="h-4 w-4 stroke-current text-slate-800 dark:text-slate-50 sm:h-5 sm:w-5 2xl:h-6 2xl:w-6"/>
                    </span>
                    {(post === null || post === void 0 ? void 0 : post.responseCount) > 0 && (<span className="ml-0.5 pr-2">{(0, image_1.kFormatter)(post.responseCount + (post.replyCount || 0))}</span>)}
                  </>
                </button>
              </div>)}
          </post_floating_bar_tooltip_wrapper_1.default>

          <separator_root_1.Separator className="mx-2 h-5"/>
          ...
          <post_share_widget_1.default post={post} shareText={shareText}/>
        </div>
      </div>
    </Tooltip.Provider>);
}
exports.default = PostFloatingMenu;
